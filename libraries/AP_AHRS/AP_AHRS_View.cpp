...
{
    switch (rotation) {
    case ROTATION_NONE: y_angle = 0; break;
    case ROTATION_PITCH_90: y_angle = 90; break;
    case ROTATION_PITCH_270: y_angle =  270; break;
    default: AP_HAL::panic("Unsupported AHRS view %u\n", (unsigned)rotation); }
...
void AP_AHRS_View::update(bool skip_ins_update)
{
    ...
    board_rotate= (board_rotate -1515) *0.2f;                   // 新加语句，转换为最大倾斜角度
    ...
    rot_body_to_ned.to_euler(&roll, &pitch, &yaw);              // 原有语句
...
{   // 此处对get_gyro_latest进行俯仰旋转，经测试验证，方法可行且稳定，可用于姿态角速率控制
    ...
    board_rotate= (board_rotate -1515) *0.2f;                   // 转换为最大倾斜角度
    board_rotation.from_euler(radians(0), radians(-board_rotate), radians(0)); // 新加语句
