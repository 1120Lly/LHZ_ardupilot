...
public:
    ...
    float cos_pitch() const { return trig.cos_pitch; }
    float cos_yaw()   const { return trig.cos_yaw;   }
    float sin_roll()  const { return trig.sin_roll;  }
    float sin_pitch() const { return trig.sin_pitch; }
    float sin_yaw()   const { return trig.sin_yaw;   }



    // wrappers around ahrs functions which pass-thru directly. See AP_AHRS.h for description of each function

    bool get_position(struct Location &loc) const WARN_IF_UNUSED { return ahrs.get_position(loc); }

    Vector3f wind_estimate(void) { return ahrs.wind_estimate(); }
