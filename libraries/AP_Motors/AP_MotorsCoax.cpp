...
void AP_MotorsCoax::init(motor_frame_class frame_class, motor_frame_type frame_type) // 初始化
{   for (uint8_t i = 0; i < 4; i++) {  add_motor_num(CH_1 + i);  }   // 确保输出4个输出通道
    ...
    _flags.initialised_ok = (frame_class == MOTOR_FRAME_COAX);       // 如果我们安装的是预期机架类型，则记录初始化成功
}

void AP_MotorsCoax::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{   _flags.initialised_ok = (frame_class == MOTOR_FRAME_COAX);   }   // 设置机架类型
...
// 获取执行器掩码：返回一个位掩码，输出1表示执行器正在使用，用于确保其他PWM输出不冲突
...
void AP_MotorsCoax::output_armed_stabilizing()                      // 给执行器发送指令
{   float   roll_thrust;                // roll thrust input value, +/- 1.0
    ...
    _actuator_out[0] = roll_thrust / thrust_out_actuator; // 执行器偏角与操纵力矩成正比，与油门成反比，即油门越大，偏角越小
